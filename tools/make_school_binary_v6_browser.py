...
from src.preprocessing.school_foreground_v4 import (
    choose_school_foreground_v6,
    global_dark_120,
    load_rgb_and_support,
    paper_ratio_dark_auto,
    paper_ratio_rgb_dark_auto,
...
def make_variant(
    ...
    if name == "school_dark_auto_v2":
        return school_dark_auto_v2(gray)

    if name == "global_dark_120":
        return global_dark_120(gray)

    if name == "paper_ratio_dark_auto":
        return paper_ratio_dark_auto(gray, support)

    if name == "paper_ratio_120":
        return paper_ratio_threshold(
            gray,
            support,
            threshold=120,
        )

    if name == "paper_ratio_135":
        return paper_ratio_threshold(
            gray,
            support,
            threshold=135,
    ...
    if name == "paper_ratio_rgb_dark_auto":
        return paper_ratio_rgb_dark_auto(
            rgb,
            support,
        )

    if name == "school_foreground_v6_auto":
        fg, _method = choose_school_foreground_v6(
            rgb,
            gray,
            support,
        )
...
def prepare_item(row: dict[str, Any], panel_w: int, panel_h: int) -> dict[str, Any]:
    ...
    for name in VARIANTS:
        try:
            ...
            skel = skeletonize(fg)
            panel = make_panel(gray, fg, name, panel_w, panel_h)
