...
def make_variant(
    ...
    if name == "school_dark_auto_v2_025":
        fg145 = cleanup_mask(
            arr < 145
        )
    ...
    if name == "percentile_otsu":
        normalized = percentile_normalized(
            arr
        )
        return otsu_dark(normalized)

    if name == "division_otsu":
        normalized = division_normalized(
            arr
        )
        return otsu_dark(normalized)

    if name == "bgdiff_otsu":
        return bgdiff_mask(arr)
