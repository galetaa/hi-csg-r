...
COCO_SOURCE: SchoolCocoSource | None = None
...
def prepare_item(row: dict[str, Any], panel_w: int, panel_h: int) -> dict[str, Any]:
    ...
    raw_row = RAW_BY_ID.get(sample_id)

    if raw_row is None:
        raise KeyError(f"No raw row for {sample_id}")

    if COCO_SOURCE is None:
        raise RuntimeError("COCO_SOURCE was not initialized")
...
def main() -> None:
    ...
    global RAW_BY_ID, COCO_SOURCE

    RAW_BY_ID = load_raw_index(Path(args.raw_root))

    COCO_SOURCE = SchoolCocoSource(args.school_raw_dir)
